...
class FormDao {
  ...
  Future<int> insertForm(FormularioOld formulario) async {
  ...
  Future<List<FormularioOld>> getForms() async {
    ...
    return result.map((map) => FormularioOld.fromMap(map)).toList();
  }

  // DAO que atualiza uma entrada de formulário do banco de dados
  Future<void> atualizarFormulario(FormularioOld formulario) async {
    final db = await AppDatabase().database;
    await db.update(
      table,
      formulario.toMap(),
